...
use lispa\amos\workflow\AmosWorkflow;
...
//        }
    ...
    public function run()
    {
        if (!$this->model->isNewRecord || $this->model->isNewRecord && $this->viewWidgetOnNewRecord) {
            ...
            if ($this->model->hasWorkflowStatus()) {
                ...
                // pulsanti
                $buttonsGot = $this->getButtonsToRender($this->initialStatusName, $this->initialStatus,
                    $this->statusToRender);
     ...
     */
    protected function getButtonsToRender($fakeStatusName = null, $fakeStatus = null, $statusToRender = null,
                                          $customStatusLabelDescription = null)
    {
            ...
            if ($state && ($key != $inState)) {
                if ($User->can($state->getId(), ['model' => $this->model])) {
                    ...
                    if ((!isset($metadati['hidden']) || (isset($metadati['hidden']) && strtolower($metadati['hidden']) != 'true'))
                        ...
                        $buttonLabel     = WorkflowTransitionWidgetUtility::getStatusButtonLabel($module, $this->model,
                                $metadati, $currentStatus, $this->translationCategory);
                        $stateDescriptor = WorkflowTransitionWidgetUtility::getStatusButtonDescription($module,
                                $this->model, $metadati, $currentStatus, $this->translationCategory);
                        ...
                        $dataConfirm = WorkflowTransitionWidgetUtility::getStatusButtonDataConfirm($module,
                                $this->model, $metadati, $currentStatus, $this->translationCategory);
                        ...
                        $buttons[] = [
                            ...
                            'order' => ((isset($metadati['order']) && is_numeric($metadati['order'])) ? $metadati['order']
                                : 0)
                        ];
                    }
                }
    ...
    {
        if (!empty(\Yii::$app->getModule('notify')) && $this->model instanceof \lispa\amos\notificationmanager\record\NotifyRecord) {
